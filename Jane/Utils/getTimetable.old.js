...
module.exports = class TimetableEmbed {
  constructor (dateToRead, timeOfSchool = 'ONLINE', showLinks = true, sClass) {
    ...
    if (dayValueArray[4]) {
      ...
      logger.info(`Cycle Number: ${cycleNumber}, odd: ${oddCycle}`)
    }
    ...
    if (cycleDay.includes('TestDay')) {
      ...
      if (cycleDay.includes('[')) {
        const splitted = cycleDay.split('+')
        lessonsString = ''
        for (const lsnString of splitted) {
          const [cycleDay, lessonNum] = lsnString.replace(']', '').split('[')
          let [from, to] = lessonNum.split('-')
          if (!to) to = Number(from)
          for (let i = Number(from) - 1; i < Number(to); i++) {
            lessonsString += `${timetableJson[sClass][cycleDay].split(' ')[i]} `
          }
        }
        lessonsString = lessonsString.replace(/\s$/, '')
      } else {
    ...
    logger.info(lessonsArray)

    if (lessonsArray[5] || lessonsArray.length === 3) {
      let timetableReadable
      let embedDesc = ''

      let timeList = classTimes.NORMAL
      if (dateToRead in classTimes) {
        timeList = classTimes[dateToRead]
      } else if (timeOfSchool in classTimes) {
        timeList = classTimes[timeOfSchool]
      }
      ...
      for (let i = 0; i < lessonsArray.length; i++) {
        ...
        if (i + 1 === lessonsArray.length) timetableReadable = embedDesc
      }
      ...
      logger.info(`2021-${monthToRead}-${dayNumberToRead}T12:00:00`)
      logger.info(JSDateForDay.getTime())
    ...
    } else if (!dayValueArray[0].includes('測驗周')) {
      const JSDateForDay = new Date(
        `2021-${toTwoDigit(monthToRead)}-${toTwoDigit(
          dayNumberToRead
        )}T12:00:00`
