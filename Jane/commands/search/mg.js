...
module.exports = class MoegirlCommand extends Command {
  ...
  async run (message, args) {
    try {
      ...
      request(
        `https://zh.moegirl.org.cn/api.php?action=query&titles=${encodeURIComponent(
          search
        ...
        (err, res, body) => {
          if (err) {
            ...
            if (err.code === 'ERR_UNESCAPED_CHARACTERS') {
              return resultmsg.edit('很抱歉, 簡 目前在萌百只能搜索英文的關鍵詞')
            }
          ...
          if (body.query.pages[id] == null) {
            return resultmsg.edit(`簡在萌百找不到${search}的頁面`)
          }
          if (body.query.pages[id].revisions == null) {
            return resultmsg.edit(`簡在萌百找不到${search}的頁面`)
          }
          if (body.query.pages[id].revisions[0] == null) {
            return resultmsg.edit(`簡在萌百找不到${search}的頁面`)
          }
          ...
          const rep = pre
            .replace(
              /{{[^人黑}]*}}|\[http.*?\u0020|\](?<=\[https[^[]*)|\[{2}[^\]]*?\||\]{2}(?<=\|[^[]*)|[[\]]|<gallery>.*<\/gallery>/gs,
              ''
            )
            .replace(/\*/g, '-') // * -> -
            ...
            .replace(/\n:/gs, '\n\u0020\u0020\u0020\u0020')
            .match(/[\s\S]{1,1900}/g)
          const final = chineseConv.tify(
            `${''}${search}\n\n${rep[0]}${''}${
              rep.length >= 2 ? '...\n查看完整內容:' : '\n網址:'
            } https://zh.moegirl.org.cn/${search}`
          )
