  ...
  async run (message, args) {
    ...
    try {
      ...
      if (reg.test(args[0]) === false) {
        return message.inlineReply('簡 只能把慢速模式調成一個數字')
      }
      if (args[0] > 21600) {
        return message.inlineReply('這個數字太大了 簡不能把慢速模式調成這個數字')
      }
      if (message.author.id === '690822196972486656' || message.author.id === '726439536401580114' || message.author.id === '794181749432778753') {
        message.channel.setRateLimitPerUser(args[0])
        message.inlineReply(`慢速模式已調成${args[0]}秒`)
      } else {
        return message.inlineReply('對不起!你不能使用此指令。')
      }
    } catch (e) {
      message.inlineReply('簡 發現了一個錯誤 .-.\n請<@690822196972486656>幫幫忙')
