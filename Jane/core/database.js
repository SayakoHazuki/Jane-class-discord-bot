...
async function add(message, action, amount) {
    try {
        Logger.info(
            `Adding ${amount} hgd to ${
                message.author?.tag || message.user?.tag
            } for ${action}`
        );
        ...
        const date = new Date();
        updateDocument.$set[`last${action}`] = Math.floor(
            date.getTime() / 1000
        );
    ...
    try {
        ...
        const sortedList = result.map((i) =>
            i.snowflake === "726439536401580114"
                ? `**## Lv.無限 ${i.tag} (簡的母親)**\n\t好感度: 無限`
                : `**#${sortedIds.indexOf(i.snowflake)} Lv.${
                      getLevel(i.hgd).value
                  } ${i.tag}**\n\t好感度: ${i.hgd}`
        );
        return sortedList;
    } catch (e) {
...
async function generateActionRow(message, userdata) {
    ...
    for (const { cmd, available } of availableActions) {
        const emoji =
            cmd.code in emojis.actionEmojis
                ? emojis.actionEmojis[cmd.code]
                : null;
        if (!emoji) continue;
...
async function getAvailableActions(message, userdata) {
    ...
    for (const cmd of commands) {
        ...
        const { diffRequirement, lvRequirement, timeRange, dayRange } =
            cmd.config;
        if (diffRequirement) {
