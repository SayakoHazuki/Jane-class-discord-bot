...
export class JaneClient extends Client {
  ...
  async registerCommands() {
    ...
    for (const commandPath of commands) {
      const _CommandBuilder: { command: typeof CommandExport } = await import(
        commandPath
      );
      try {
    ...
    try {
      ...
      await rest.put(
        ...
        {
          body: Array.from(
            this.commands.map((command) => command.slashCommandData).values()
          ),
        }
      );
  ...
  async registerEvents() {
    ...
    for (const event of events) {
      const _EventBuilder: { event: typeof EventExport } = await import(event);
